...
def main():
    parser = argparse.ArgumentParser('bnf-VC trainer with ResidualNet')
    ...
    model = BLSTMResConversionModel(in_channels=hps.Audio.bn_dim + 2,
                                    ...
                                    other_params=hps.ResidualNet.other_params)
    model.to(device)
    ...
    for epoch in range(hps.TrainToOne.epochs):
        ...
        torch.save(model.state_dict(), os.path.join(args.model_dir, "bnf-vc-to-one-res-{}.pt".format(epoch)))
        # validation
        model.eval()
        dev_running_loss = 0.
